...
class CrowdTangle(API):
    ...
    def flatten(self, post):
        ...
        if self.endpoint == 'post' and self.history:
            if 'history' in post and isinstance(post['history'], list):
                timestep = [timestamp['timestep'] if 'timestep' in timestamp else "" for
                            timestamp in post['history']]
                date = [timestamp['date'] if 'date' in timestamp else "" for timestamp
                        in post['history']]
                score = [timestamp['score'] if 'score' in timestamp else "" for
                         timestamp in post['history']]
                historyActualLikeCount = [
                    timestamp['actual']['likeCount'] if 'likeCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyActualShareCount = [
                    timestamp['actual']['shareCount'] if 'shareCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyActualCommentCount = [
                    timestamp['actual']['commentCount'] if 'commentCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyActualLoveCount = [
                    timestamp['actual']['loveCount'] if 'loveCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyActualWowCount = [
                    timestamp['actual']['wowCount'] if 'wowCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyActualHahaCount = [
                    timestamp['actual']['hahaCount'] if 'hahaCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyActualSadCount = [
                    timestamp['actual']['sadCount'] if 'sadCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyActualAngryCount = [
                    timestamp['actual']['angryCount'] if 'angryCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyActualThankfulCount = [
                    timestamp['actual']['thankfulCount'] if 'thankfulCount' in
                                                            timestamp[
                                                                'actual'] else row.append(
                        "") for timestamp in post['history']]
                historyActualCareCount = [
                    timestamp['actual']['careCount'] if 'careCount' in timestamp[
                        'actual'] else row.append("") for timestamp in post['history']]
                historyExpectedLikeCount = [
                    timestamp['expected']['likeCount'] if 'likeCount' in timestamp[
                        'expected'] else row.append("") for timestamp in
                    post['history']]
                historyExpectedShareCount = [
                    timestamp['expected']['shareCount'] if 'shareCount' in timestamp[
                        'expected'] else row.append("") for timestamp in
                    post['history']]
                historyExpectedCommentCount = [
                    timestamp['expected']['commentCount'] if 'commentCount' in
                                                             timestamp[
                                                                 'expected'] else row.append(
                        "") for timestamp in post['history']]
                historyExpectedLoveCount = [
                    timestamp['expected']['loveCount'] if 'loveCount' in timestamp[
                        'expected'] else row.append("") for timestamp in
                    post['history']]
                historyExpectedWowCount = [
                    timestamp['expected']['wowCount'] if 'wowCount' in timestamp[
                        'expected'] else row.append("") for timestamp in
                    post['history']]
                historyExpectedHahaCount = [
                    timestamp['expected']['hahaCount'] if 'hahaCount' in timestamp[
                        'expected'] else row.append("") for timestamp in
                    post['history']]
                historyExpectedSadCount = [
                    timestamp['expected']['sadCount'] if 'sadCount' in timestamp[
                        'expected'] else row.append("") for timestamp in
                    post['history']]
                historyExpectedAngryCount = [
                    timestamp['expected']['angryCount'] if 'angryCount' in timestamp[
                        'expected'] else row.append("") for timestamp in
                    post['history']]
                historyExpectedThankfulCount = [
                    timestamp['expected']['thankfulCount'] if 'thankfulCount' in
                                                              timestamp[
                                                                  'expected'] else row.append(
                        "") for timestamp in post['history']]
                historyExpectedCareCount = [
                    timestamp['expected']['careCount'] if 'careCount' in timestamp[
                        'expected'] else row.append("") for timestamp in
                    post['history']]
            else:
                timestep = ""
    ...
    def run(self):
        ...
        self.runTimeframes()
        append_to_bq(self.bq_credential, self.bq_table_id,
                     self.output_filename) if self.togbq else None
    ...
    def runTimeframes(self):
        if self.endpoint == "posts/search":
            ...
            for i in range(len(self.links)):
                ...
                res = self.linksEndpoint(link=self.links[i],
    ...
    def writeDataToCSV(self, data):
        with open(self.output_filename, 'a', encoding='utf-8', errors='ignore',
                  newline='') as f:
            writer = csv.writer(f)
            writer.writerows(data)
            self.earliestStartDate = datetime.datetime.fromisoformat(
                data[-1][2])  # 2 is the position of date
